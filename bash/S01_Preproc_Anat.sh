...
SES_DIR=`echo ${SBJ_DIR}/${SES}`
ANAT_DATA_DIR=`echo ${SES_DIR}/D01_Anatomical`

echo "++ Working on Subject ${SBJ} | Session ${SES} ... pre-processing anatomical data"
...
if [ ! -e ${ANAT_DATA_DIR}/${SBJ}_Anat+orig.HEAD ]; then
   ...
   3dcopy -overwrite ${PRJDIR}/freesurfer/${SBJ}_${SES}/SUMA/T1.nii.gz ${SBJ}_Anat+orig
   3dcopy -overwrite ${PRJDIR}/freesurfer/${SBJ}_${SES}/SUMA/fs_parc_wb_mask.nii.gz ${SBJ}_Anat_FBmask+orig
fi
